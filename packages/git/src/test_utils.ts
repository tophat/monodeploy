...
export async function setupTestRepository(): string {
...
export async function cleanUp(paths: string[]): void {
    await Promise.all(paths.map(path => fs.rmdir(path, { recursive: true })))
}

export async function createFile(filePath: string, cwd: string): void {
